...
- **Model deployments are variable-driven, not guessed.** Three required deployments
  (`gpt-5.6-luna` = Prompt/Hosted Agents,
  `gpt-5.5` = Foundry IQ, Lab 5 configurable LLM judges, and both Agent Optimizer roles,
  `embedding` = text-embedding-3-small) have overridable
  model-name/version/sku/capacity variables; deployment names are fixed.
  Both chat model versions intentionally have no default.
  `scripts/preflight.sh` discovers each version and quota `usageName`
  ...
  The shared GPT-5.5 allocation is 100 to cover Foundry IQ, evaluation, and optimization.
  ...
  required deployment and always contain its concrete name.
  A Terraform `moved` block preserves existing state when upgrading the former
  indexed optional deployment to the required unindexed resource.
...
- Exact Luna/GPT-5.5 versions are intentionally not hardcoded. Both must be
  resolved by `scripts/preflight.sh`, including GPT-5.5's 100K allocation.
  Model catalog/quota checks do not prove Portal
  picker support.
