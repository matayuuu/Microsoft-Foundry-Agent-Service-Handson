...
- Azure Portal: participant creates/deletes exactly one workload resource group and
  later creates/deletes the Azure ML Compute instance.
- Azure Cloud Shell Bash: provisioning/download only, from a verified persistent HOME.
...
- On first launch, let the standard Cloud Shell UI create persistent storage, then
  verify the Azure Files-backed HOME before Lab 1 timing starts.
- A failed/unverifiable HOME mount is a hard stop. Never provision from ephemeral HOME.
- Repository, `.venv`, `.workshop`, and Terraform state remain under persisted Unix HOME,
  not directly on `clouddrive`.
...
Export notebooks; stop/delete Compute; reopen the same persistent Cloud Shell repository,
source activation, run `./scripts/destroy.sh`; verify workload RG empty; delete it in
Azure Portal; `exit`. Cloud Shell storage has a separate lifecycle.
