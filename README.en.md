...
**Luna (`gpt-5.6-luna`)** is shared by the Prompt and Hosted Agent runtimes;
**GPT-5.5 (`gpt-5.5`)** is shared by Foundry IQ, the configurable evaluation judges
in Lab 5, and both the Evaluation and Optimization model roles in Lab 6;
**`text-embedding-3-small`**, deployed as `embedding`, converts text into numbers for search.
All three models' `GlobalStandard` SKU and required capacity are prerequisites for the
full workshop.
